-- Codex CLI can export an unset OTLP timeUnixNano as "0" while still
-- providing a valid event.timestamp. The first collector version preferred
-- that zero clock, so otherwise valid Codex usage landed outside the 35-day
-- Project usage window. Move only those impossible legacy periods to their
-- actual ingestion hour, merging safely with any already-correct bucket.
