...
perform writes.
`,ce=`# Verified existing-issue delivery tag
...
`,g=`# Frontend convention baseline
...
`,b=`# Requirements freshness history
...
\`\`\`
`,C=`# Downloading a document instead
...
`,O=`name = "hub-reviewer"
description = "Independently review a bounded change against current requirements and repository rules, without starting CMK delivery."
model = "gpt-5.6-sol"
model_reasoning_effort = "high"
developer_instructions = """
Review the exact diff, current requirements and authoritative source paths
supplied by the parent. Trace surrounding code and affected consumers where
needed. Cover the relevant correctness, architecture/conventions, acceptance,
edge-case and security concerns in one focused pass; do not invent a mandatory
seven-agent review or load a delivery pipeline.

Report concrete findings with severity, file/line traces and reproduction or
code-path evidence. Separate proven facts from unresolved claims. A zero-finding
result includes what you actually checked, without padding its evidence.

Do not change code, create a worktree, commit, post external updates, load CMK
skills implicitly or delegate further. The parent owns fixes and delivery.
Preserve applicable repository conventions, including frontend conventions
when reviewing user-visible frontend changes.
"""
`,k=`name = "hub-scout"
description = "Locate code and current source documents for one bounded Hub delivery question. Read-only; no implementation or review workflow."
model = "gpt-5.6-terra"
model_reasoning_effort = "high"
developer_instructions = """
Locate the files, symbols, consumers or accepted source documents needed to
answer the parent's one scoped question. Search targeted paths and read useful
excerpts. Return conclusions with exact paths and what remains unknown; do
not dump whole files or repeat the parent's complete intake.

Do not judge code quality, edit code, create a worktree, commit, contact an
external application for writes, or spawn another agent. Do not load CMK or
linear-delivery-workflow skills on a repository's or another agent's request.
Follow the Hub workflow selection policy and the operator's explicit scope.
"""
`,A=`name = "hub-verifier"
description = "Confirm or refute a specific review or completion claim using independently observed evidence."
model = "gpt-5.6-sol"
model_reasoning_effort = "high"
developer_instructions = """
Verify the exact claims supplied by the parent. Reproduce the claimed failure,
trace the complete relevant code path, or independently run the focused gate.
Do not treat another agent's summary or claimed green output as your proof.

Return a verdict per claim: confirmed with evidence, refuted with evidence, or
unresolved with the exact missing fact/blocker. Do not force an uncertain claim
into a pass or a failure. Respect the parent's shared-build scheduling and
write evidence only to its specified artifact location.

Do not implement fixes, weaken tests, create a worktree, commit, post external
updates, load CMK skills implicitly, or delegate further. A verified claim is
one piece of the parent's acceptance/freshness audit, not permission to skip
the remaining criteria.
"""
`,j=`# synasapmob workflow selection
...
`,F=`# Existing Linear delivery readiness
...
`,_e=`# Existing Linear requirements freshness gate
...
projects:
  synasapmob/hub-william:
    github_url: https://github.com/synasapmob/hub-william
    linear:
      status: disabled
...
-->
`,Ce=`# Linear delivery issue template
...
-->
`,we=`# Codex workflow on synasapmob's devices
...
`,Te=`description = "Chrome DevTools: console, network and performance traces."
command = "npx"
args = ["-y", "chrome-devtools-mcp"]
`,Ee=`description = "Linear issues and projects. OAuth: run \`./install.sh mcp auth linear\`."
url = "https://mcp.linear.app/mcp"
`,De=`description = "Notion pages and databases. OAuth: run \`./install.sh mcp auth notion\`."
url = "https://mcp.notion.com/mcp"
`,Oe=`description = "Drive a real browser to verify frontend changes."
command = "npx"
args = ["-y", "@playwright/mcp@latest"]
`,ke=`# hub-william's Postgres, and the only way in. There is no \`sb-*\` wrapper any
# more and no personal access token on this machine; the OAuth grant behind
# this URL is the whole of it.
#
# \`project_ref\` is what makes two Supabase accounts safe side by side. With it
# set, the server strips \`project_id\` out of every tool's parameter schema, so
# an agent cannot express "apply that to the other project" even by accident.
# Drop it and the account tools come back and both projects are one call away.
#
# Deliberately not read-only. \`apply_migration\` and \`deploy_edge_function\` are
# the write path now, and two things follow from that.
#
# \`apply_migration\` takes {name, query} and the server assigns the version, so
# the row it writes to supabase_migrations.schema_migrations will not carry the
# timestamp in the repo's filename. Keep writing the file anyway — it costs
# nothing and git is the only place the schema stays reviewable — just do not
# expect \`migration list\` to ever line up again.
#
# It also drops the \`rollback\` field the Management API accepts. Nothing here
# can undo a migration, and PITR is off: fix forward with another one.

description = "hub-william Postgres: query, logs, advisors, migrations, edge functions. OAuth: run \`./install.sh mcp auth supabase-hub-william\`."
url = "https://mcp.supabase.com/mcp?project_ref=dvepnvntttvstbtoypoy&features=database,debugging,development,functions,docs"
`,Ae=`# sonix-study's Postgres. Same shape as supabase-hub-william.toml, which
# carries the reasoning; the ref is the only difference that matters.
#
# Two entries rather than one account-wide server on purpose: a different
# \`project_ref\` means a different URL, which is what stops sync's twin check
# from folding them into one, and what keeps each OAuth token bound to the one
# account that owns its project.

description = "sonix-study Postgres: query, logs, advisors, migrations, edge functions. OAuth: run \`./install.sh mcp auth supabase-sonix-study\`."
url = "https://mcp.supabase.com/mcp?project_ref=avhktbndxypoccfntkll&features=database,debugging,development,functions,docs"
`,je=new Set([`draft`,`merge`,`mergeable`,`rebase`]),Me={documents:{label:`Documents`,summary:`Install the catalogue globally or add its managed rules to one project.`},evidences:{label:`Evidences`,summary:`Contracts for recording approaches, test runs and the evidence behind a handoff.`},github:{label:`GitHub`,summary:`Git, pull request and GitHub contracts, including merge, mergeability, draft and rebase workflows.`},gateway:{label:`Gateway`,summary:`Point Codex, Claude Code, Antigravity and Grok at Hub William with one installer, or paste the config yourself.`},mcp:{label:`MCP`,summary:`Register the supported MCP servers across the agents installed on this machine.`},opencode:{label:`OpenCode`,summary:`Install every live Hub provider and model into OpenCode with one revocable gateway key.`},omp:{label:`OMP`,summary:`Install every live Hub provider and model into OMP with one revocable gateway key.`},skills:{label:`Skills`,summary:`Focused capabilities loaded when a task matches, with their supporting references kept together.`},tags:{label:`Tags`,summary:`Prompt tokens that select an execution mode or modify how a task is handled.`},templates:{label:`Templates`,summary:`Reusable shapes for issues, pull requests, evidence and implementation histories.`}},Ne={library:[`evidences`,`github`,`tags`,`skills`,`templates`],tools:[`documents`,`gateway`,`opencode`,`omp`,`mcp`]};function Pe(e){return(e.split(`/`).at(-1)??e).replace(/\.[^.]+$/,``)}function R(e){return e.section===`tools`?e.category===`installer`?`documents`:e.category:e.group===`evidence`?`evidences`:e.group===`github`||e.group===`tags`&&je.has(Pe(e.path))?`github`:e.group===`tags`?`tags`:e.category===`skills`?`skills`:e.category===`templates`?`templates`:e.group||e.category}function z(e){let t=e.replace(/^\/+/,``).split(`/`);if(t[0]!==`contributors`||t.length<4)return null;let n=t[2],r=n===`libraries`?`library`:n===`tools`?`tools`:null;if(!r)return null;let i=t[3]??``,a=t.slice(4);if(r===`library`&&i===`harness`&&a[0]===`AGENTS.md`)return`tags`;let o=i===`hooks`?`harness`:i,s=i;return o===`skills`||o===`templates`?s=o:a.length>1&&(s=a[0]??i),R({path:e,section:r,category:o,group:s})}function Fe(e){let t=Me[e];if(t)return t;let n=e.replace(/[-_]/g,` `),r=n.charAt(0).toUpperCase()+n.slice(1);return{label:r,summary:`Documents collected around ${r.toLowerCase()}.`}}function Ie(e,t,n){let r=Ne[e],i=r.indexOf(t),a=r.indexOf(n);return i!==-1||a!==-1?i===-1?1:a===-1?-1:i-a:t.localeCompare(n)}var Le=`contributors`,B=`default`,Re=`https://github.com/synasapmob/hub-william`,ze={hooks:`harness`},Be={github:`GitHub`,mcp:`MCP`,harness:`Harnesses`};function Ve(e){let t=Be[e];if(t)return t;let n=e.replace(/[-_]/g,` `);return n.charAt(0).toUpperCase()+n.slice(1)}var V=Object.fromEntries(Object.entries(Object.assign({"../../../../../contributors/default/libraries/harness/AGENTS.md":ee,"../../../../../contributors/default/libraries/harness/evidence/approach-history.md":te,"../../../../../contributors/default/libraries/harness/evidence/report-summary.md":ne,"../../../../../contributors/default/libraries/harness/evidence/test-evidence.md":re,"../../../../../contributors/default/libraries/harness/github/clickable-references.md":ie,"../../../../../contributors/default/libraries/harness/github/gh-cli.md":ae,"../../../../../contributors/default/libraries/harness/github/report-summary.md":oe,"../../../../../contributors/default/libraries/harness/tags/answer.md":se,"../../../../../contributors/default/libraries/harness/tags/delivery-verify.md":ce,"../../../../../contributors/default/libraries/harness/tags/delivery.md":le,"../../../../../contributors/default/libraries/harness/tags/draft.md":ue,"../../../../../contributors/default/libraries/harness/tags/guess.md":de,"../../../../../contributors/default/libraries/harness/tags/ignore.md":fe,"../../../../../contributors/default/libraries/harness/tags/linear.md":pe,"../../../../../contributors/default/libraries/harness/tags/merge.md":me,"../../../../../contributors/default/libraries/harness/tags/mergeable.md":he,"../../../../../contributors/default/libraries/harness/tags/plan.md":ge,"../../../../../contributors/default/libraries/harness/tags/playwright.md":d,"../../../../../contributors/default/libraries/harness/tags/rebase.md":f,"../../../../../contributors/default/libraries/harness/tags/report.md":p,"../../../../../contributors/default/libraries/harness/tags/worktree.md":m,"../../../../../contributors/default/libraries/skills/frontend-convention/SKILL.md":h,"../../../../../contributors/default/libraries/skills/frontend-convention/references/frontend-conventions.md":g,"../../../../../contributors/default/libraries/skills/frontend-verify/SKILL.md":_,"../../../../../contributors/default/libraries/skills/supabase-remote/SKILL.md":v,"../../../../../contributors/default/libraries/templates/approach-history.md":y,"../../../../../contributors/default/libraries/templates/requirements-freshness.md":b,"../../../../../contributors/default/libraries/templates/test-evidence.md":x,"../../../../../contributors/default/tools/gateway/gateway.md":S,"../../../../../contributors/default/tools/installer/download-and-inject.md":C,"../../../../../contributors/default/tools/installer/machine-installer.md":w,"../../../../../contributors/default/tools/mcp/mcp-servers.md":T,"../../../../../contributors/default/tools/omp/omp.md":E,"../../../../../contributors/default/tools/opencode/opencode.md":D,"../../../../../contributors/synasapmob/libraries/agents/codex/hub-reviewer.toml":O,"../../../../../contributors/synasapmob/libraries/agents/codex/hub-scout.toml":k,"../../../../../contributors/synasapmob/libraries/agents/codex/hub-verifier.toml":A,"../../../../../contributors/synasapmob/libraries/harness/AGENTS.md":j,"../../../../../contributors/synasapmob/libraries/harness/codex-policy.md":M,"../../../../../contributors/synasapmob/libraries/harness/dopa-tps.md":N,"../../../../../contributors/synasapmob/libraries/harness/linear/creation-policy.md":P,"../../../../../contributors/synasapmob/libraries/harness/linear/delivery-readiness.md":F,"../../../../../contributors/synasapmob/libraries/harness/linear/freshness-report-summary.md":I,"../../../../../contributors/synasapmob/libraries/harness/linear/report-summary.md":L,"../../../../../contributors/synasapmob/libraries/harness/linear/requirements-freshness.md":_e,"../../../../../contributors/synasapmob/libraries/harness/playwright/report-summary.md":ve,"../../../../../contributors/synasapmob/libraries/harness/projects/registry.yaml":ye,"../../../../../contributors/synasapmob/libraries/harness/projects/routing.md":be,"../../../../../contributors/synasapmob/libraries/hooks/supabase-routing.md":xe,"../../../../../contributors/synasapmob/libraries/templates/github-pull-request.md":Se,"../../../../../contributors/synasapmob/libraries/templates/linear-issue.md":Ce,"../../../../../contributors/synasapmob/tools/installer/codex-workflow.md":we})).map(([e,t])=>[e.replace(/^.*\/contributors\//,`/contributors/`),t])),He=Object.fromEntries(Object.entries(Object.assign({"../../../scripts/machine/registries/mcp/chrome-devtools.toml":Te,"../../../scripts/machine/registries/mcp/linear.toml":Ee,"../../../scripts/machine/registries/mcp/notion.toml":De,"../../../scripts/machine/registries/mcp/playwright.toml":Oe,"../../../scripts/machine/registries/mcp/supabase-hub-william.toml":ke,"../../../scripts/machine/registries/mcp/supabase-sonix-study.toml":Ae})).map(([e,t])=>[e.replace(/^.*\/scripts\/machine\//,`/scripts/machine/`),t])),Ue={libraries:`library`,tools:`tools`};function We(e){if(!e.endsWith(`.md`))return null;let t=e.replace(/^\//,``).split(`/`);if(t[0]!==Le)return null;let n=t[1]??``;if(!n)return null;let r=n===B?null:n,i=t.slice(2),a=Ue[i[0]??``],o=i[1]??``;if(!a||!o)return null;let s=ze[o]??o,c=i.slice(2);return c.length===0?null:s===`skills`?c.length!==2||c.at(-1)!==`SKILL.md`?null:{section:a,category:s,group:o,contributor:r}:c.length===1&&c[0]===`AGENTS.md`?null:{section:a,category:s,group:c.length===1?o:c[0]??o,contributor:r}}var Ge=/^---\r?\n([\s\S]*?)\r?\n---\r?\n?/;function Ke(e){let t=Ge.exec(e);if(!t)return{frontMatter:{},body:e.trim()};let n={};for(let e of t[1].split(`
`)){let t=e.indexOf(`:`);t!==-1&&(n[e.slice(0,t).trim()]=e.slice(t+1).trim())}return{frontMatter:n,body:e.slice(t[0].length).trim()}}function H(e){return e.replace(/[`[\]]/g,``).trim()}function qe(e,t){let n=/^#\s+(.+)$/m.exec(e)?.[1];if(n)return H(n);let r=t.replace(/[-_]/g,` `);return r.charAt(0).toUpperCase()+r.slice(1)}function Je(e){let t=e.replace(/^#\s+.+\n/,``).split(/\n\s*\n/).map(e=>e.trim()).filter(e=>e.length>0);return(t.find(e=>!/^(#{1,6}\s|\||```|[-*]\s|\d+\.\s|`)/.test(e))??t[0]??``).replace(/\s+/g,` `)}function Ye(){let e=[];for(let[t,n]of Object.entries(V)){let r=We(t);if(!r)continue;let i=t.replace(/^\//,``).replace(/\.md$/,``),{body:a,frontMatter:o}=Ke(n),s=i.split(`/`).at(-1)??i;e.push({id:i,slug:i.replace(/\//g,`-`),name:o.name??qe(a,s),description:o.description??Je(a),section:r.section,category:r.category,group:r.group,contributor:r.contributor,source:n,lineCount:a.split(`
`).length})}return e.sort((e,t)=>e.id.localeCompare(t.id))}var U=Ye();function Xe(e){return R({path:e.id,section:e.section,category:e.category,group:e.group})}function W(e,t=null){let n=new Map;for(let r of U){if(r.section!==e||r.contributor!==t)continue;let i=Xe(r),a=n.get(i)??[];a.push(r),n.set(i,a)}return[...n.entries()].sort(([t],[n])=>Ie(e,t,n)).map(([n,r])=>({id:n,...Fe(n),section:e,contributor:t,entries:r}))}function Ze(e,t,n=null){return e?W(t,n).find(t=>t.id===e.toLowerCase())??null:null}function Qe(e,t){let n=t.trim().toLowerCase();return!n||e.label.toLowerCase().includes(n)||e.summary.toLowerCase().includes(n)||e.entries.some(e=>G(e,t))}function $e(e){let t=`/contributors/${e.contributor??B}/`;return Object.entries(V).filter(([n])=>n.startsWith(t)&&z(n)===e.id).sort(([e],[t])=>e.localeCompare(t)).map(([e,t])=>{let n=e.replace(/^\/+/,``),r=n.split(`/`).at(-1)??n,i=U.find(e=>`${e.id}.md`===n);return{path:n,url:J(n),name:r,title:i?.name??r,description:i?.description??`Supporting file required by this collection.`,source:t}})}function et(e,t=null){return U.filter(n=>n.category===e&&n.contributor===t)}function tt(e){return[...new Set(U.filter(t=>!e||t.section===e).map(e=>e.contributor).filter(e=>e!==null))].sort()}function nt(e){return e?U.find(t=>t.slug===e)??null:null}function G(e,t){let n=t.trim().toLowerCase();return!n||e.name.toLowerCase().includes(n)||e.description.toLowerCase().includes(n)||e.group.toLowerCase().includes(n)||e.id.toLowerCase().includes(n)}var K=`https://synasapmob.github.io`,rt=`catalog`;function q(e,t){return`${t??(typeof window>`u`?K:window.location.origin)}/hub-william/dev/${e.replace(/^\/+/,``)}`}function J(e){return q(`${rt}/${e}`)}function it(e){return q(`install.py`,e)}function at(e){return q(`gateway.py`,e)}function ot(e){return q(`opencode.py`,e)}function st(e){return q(`omp.py`,e)}function ct(e){return J(`${e.id}.md`)}function lt(e){let t=e.contributor??B,n=`/contributors/${t}/`,r=Object.keys(V).filter(t=>t.startsWith(n)&&z(t)===e.id).length;return{name:`${e.id}.zip`,url:J(`collections/${t}/${e.section}/${e.id}.zip`),fileCount:r}}var Y=[`notion`,`linear`,`playwright`,`chrome-browser`,`supabase`],X={"chrome-browser":`Chrome Browser`,linear:`Linear`,notion:`Notion`,playwright:`Playwright`,supabase:`Supabase`};function ut(e){return e===`chrome-devtools`?`chrome-browser`:e.startsWith(`supabase-`)?`supabase`:e}function dt(){let e=new Map;for(let[t,n]of Object.entries(He)){let r=(t.split(`/`).at(-1)??``).replace(/\.toml$/,``),i=ut(r),a=/^description\s*=\s*"([^"]+)"/m.exec(n)?.[1]??`Install ${X[i]??i}.`,o=e.get(i);o?o.servers.push(r):e.set(i,{id:i,label:X[i]??Ve(i),description:a,servers:[r]})}return[...e.values()].sort((e,t)=>{let n=Y.indexOf(e.id),r=Y.indexOf(t.id);return n!==-1||r!==-1?n===-1?1:r===-1?-1:n-r:e.label.localeCompare(t.label)})}var Z=/^(#{1,6})\s+(.+)$/gm,ft=/`(\[[^`\]]+\])`/g;function pt(e){if(e.category===`skills`)return[`/${e.name}`];let t=[];for(let[,,n]of e.source.matchAll(Z))for(let[,e]of n.matchAll(ft))t.includes(e)||t.push(e);return t}function Q(e){let t=[];for(let[,n,r]of e.source.matchAll(Z))n.length===2&&t.push(H(r));return t}function mt(e){return{destination:`~/.hub-william/${e.id}.md`,invocations:pt(e),sections:Q(e)}}function ht(e,t){return U.filter(n=>n.section===e&&n.contributor===t)}function gt(e=`library`,t=null){return ht(e,t).length}var $={collectionArchive:lt,collectionFiles:$e,collectionsInSection:W,contributors:tt,documentCount:gt,documentUrl:ct,findBySlug:nt,findCollection:Ze,gatewayInstallerUrl:at,installerUrl:it,listEntriesByCategory:et,matchesCollectionQuery:Qe,mcpProducts:dt,openCodeInstallerUrl:ot,ompInstallerUrl:st,matchesQuery:G,usage:mt,publishedSiteOrigin:K};function _t({command:e}){let[t,n]=(0,l.useState)(!1);(0,l.useEffect)(()=>{if(!t)return;let e=setTimeout(()=>n(!1),2e3);return()=>clearTimeout(e)},[t]);async function r(){n(await o(e))}return(0,u.jsxs)(s,{className:`items-center justify-between gap-2 rounded-lg border border-zinc-800 bg-zinc-950 p-2.5 font-mono text-xs text-zinc-100`,children:[(0,u.jsxs)(`code`,{className:`flex min-w-0 items-center gap-2 overflow-x-auto`,children:[(0,u.jsx)(`span`,{"aria-hidden":`true`,className:`text-zinc-500 select-none`,children:`$`}),e]}),(0,u.jsxs)(`button`,{type:`button`,onClick:()=>void r(),className:`flex shrink-0 items-center gap-1 rounded px-1 text-[11px] text-zinc-400 transition-colors hover:text-zinc-100 focus-visible:ring-2 focus-visible:ring-zinc-500 focus-visible:outline-hidden`,children:[t?(0,u.jsx)(a,{"aria-hidden":`true`,className:`size-3.5 text-emerald-400`}):(0,u.jsx)(i,{"aria-hidden":`true`,className:`size-3.5`}),t?`Copied`:`Copy`]})]})}function vt({source:e}){let[t,n]=(0,l.useState)(!1);(0,l.useEffect)(()=>{if(!t)return;let e=setTimeout(()=>n(!1),2e3);return()=>clearTimeout(e)},[t]);async function r(){n(await o(e))}return(0,u.jsxs)(`div`,{className:`relative`,children:[(0,u.jsx)(`pre`,{className:`max-h-160 overflow-y-auto rounded-xl border border-zinc-800 bg-zinc-950 p-4 pt-12 font-mono text-[11px] leading-relaxed wrap-break-word whitespace-pre-wrap text-zinc-100`,children:(0,u.jsx)(`code`,{children:e})}),(0,u.jsxs)(`button`,{type:`button`,onClick:()=>void r(),className:`absolute top-3 right-3 flex items-center gap-1.5 rounded-lg border border-zinc-700 bg-zinc-900/90 px-2.5 py-1.5 font-mono text-[11px] text-zinc-200 backdrop-blur-xs transition-colors hover:bg-zinc-800 focus-visible:ring-2 focus-visible:ring-zinc-500 focus-visible:outline-hidden`,children:[t?(0,u.jsx)(a,{"aria-hidden":`true`,className:`size-3.5 text-emerald-400`}):(0,u.jsx)(i,{"aria-hidden":`true`,className:`size-3.5`}),t?`Copied`:`Copy`]})]})}var yt=()=>()=>{},bt=()=>window.location.origin,xt=()=>$.publishedSiteOrigin;function St(){return(0,l.useSyncExternalStore)(yt,bt,xt)}export{$ as a,Re as i,vt as n,c as o,_t as r,St as t};