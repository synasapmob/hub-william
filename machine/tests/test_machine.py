...
The seven numbered cases come straight from docs/machine-harness-plan.md and
...
class TestSkills(MachineTest):
    ...
    def test_delete_catalog_removes_the_source(self):
        ...
        self.assertEqual(result.code, 0, result)
        self.assertFalse(os.path.isdir(self.m.catalog("skills", "supabase-remote")))
...
class TestUpdateAndBootstrap(MachineTest):
    ...
    def test_update_refuses_to_pull_onto_your_own_edits(self):
        self.init()
        self.as_checkout(status=" M machine/catalog/groups.toml")
        result = self.m.run("update", "--yes")
        self.assertEqual(result.code, 1, result)
        self.assertIn("uncommitted changes", result.out)
        self.assertIn("machine/catalog/groups.toml", result.out)
    ...
    def test_boot_sh_is_valid_shell_and_hands_over_a_terminal(self):
        ...
        self.assertIn("sparse-checkout set --no-cone '/machine/'", text)
    ...
    def test_boot_sh_runs_the_command_you_gave_it(self):
        ...
        os.makedirs(os.path.join(fake, "machine"))
        installer = os.path.join(fake, "machine", "install.sh")
...
class TestHarnessAndZsh(MachineTest):
    def test_legacy_catalog_symlinks_migrate_to_generated_copies(self):
        ...
        os.symlink(self.m.catalog("harness", "AGENTS.md"), harness)
        os.symlink(self.m.catalog("skills", "frontend-verify"), skill)
    ...
    def test_global_harness_contains_the_personal_tag_contract(self):
        root_text = self.m.read(self.m.catalog("harness", "AGENTS.md"))
        tag_files = (
        ...
        tag_dir = self.m.catalog("harness", "tags")
        self.assertEqual(
        ...
        contrib_text = self.m.read(
            self.m.catalog("contrib", "synasapmob", "harness", "dopa-tps.md")
        )
        support_files = (
            ("github", "gh-cli.md"),
        ...
        support_text = "\n".join(
            self.m.read(self.m.catalog("harness", *parts))
            for parts in support_files
        )
        support_text += "\n" + self.m.read(
            self.m.catalog(
                "contrib", "synasapmob", "hooks", "supabase-routing.md"
            )
        )
        text = (
            root_text + "\n" + tag_text + "\n" + contrib_text + "\n" + support_text
        ...
        self.assertIn("`../contrib/synasapmob/harness/dopa-tps.md`", root_text)
        ...
        self.assertIn("`linear/creation-policy.md`", root_text)
        self.assertIn("`linear/delivery-readiness.md`", root_text)
        self.assertIn("`linear/requirements-freshness.md`", root_text)
        self.assertIn("`linear/freshness-report-summary.md`", root_text)
        self.assertIn("`linear/report-summary.md`", root_text)
        self.assertIn("`playwright/report-summary.md`", root_text)
        self.assertIn("`projects/routing.md`", root_text)
        self.assertIn("`projects/registry.yaml`", root_text)
        self.assertIn("contrib/synasapmob/hooks/supabase-routing.md", root_text)
        ...
        self.assertIn(
            "/Users/synasapmob/.hub-william/machine/catalog/templates/"
            "test-evidence.md",
            text,
        )
        self.assertIn(
    ...
    def test_test_evidence_template_preserves_failure_history_and_comparisons(self):
        template = self.m.read(
            self.m.catalog("templates", "test-evidence.md")
        )
    ...
    def test_approach_history_template_is_append_only_and_cross_surface(self):
        template = self.m.read(
            self.m.catalog("templates", "approach-history.md")
        )
    ...
    def test_requirements_freshness_template_is_append_only_and_traceable(self):
        template = self.m.read(
            self.m.catalog("templates", "requirements-freshness.md")
        )
    ...
    def test_project_registry_uses_direct_destinations_without_cached_branch(self):
        registry = self.m.read(
            self.m.catalog("harness", "projects", "registry.yaml")
        )
    ...
    def test_quick_guide_uses_canonical_delivery_and_answer_tags(self):
        ...
        self.assertIn("machine/catalog/harness/tags/", guide)
        ...
        self.assertIn("harness/linear/creation-policy.md", guide)
        self.assertIn("harness/linear/delivery-readiness.md", guide)
        self.assertIn("harness/linear/requirements-freshness.md", guide)
        self.assertIn("harness/linear/freshness-report-summary.md", guide)
        self.assertIn("harness/linear/report-summary.md", guide)
        ...
        self.assertIn("harness/playwright/report-summary.md", guide)
        self.assertIn("harness/projects/routing.md", guide)
        self.assertIn("harness/projects/registry.yaml", guide)
        self.assertIn("contrib/synasapmob/hooks/supabase-routing.md", guide)
        ...
        self.assertIn("machine/catalog/skills/backend-convention/", guide)
        self.assertIn(
    ...
    def test_frontend_convention_is_an_installable_catalog_skill(self):
        skill = self.m.catalog("skills", "frontend-convention", "SKILL.md")
        self.assertIn("name: frontend-convention", self.m.read(skill))
        reference = self.m.catalog(
            "skills", "frontend-convention", "references",
    ...
    def test_delivery_templates_are_local_catalog_sources(self):
        pr = self.m.read(self.m.catalog("templates", "github-pull-request.md"))
        linear = self.m.read(self.m.catalog("templates", "linear-issue.md"))
