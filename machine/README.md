...
`catalog/harness/AGENTS.md`. `init`, `sync`, and `update` generate a read-only
...
`catalog/templates/github-pull-request.md` and
`catalog/templates/linear-issue.md`. The first is vendored from dopamint-arena's
...
```sh
~/.hub-william/machine/install.sh init       # browse the catalog and pick
~/.hub-william/machine/install.sh update     # pull, then sync
~/.hub-william/machine/install.sh mcp auth   # log in to the OAuth servers
~/.hub-william/machine/install.sh status     # catalog, CLIs, profile, pending
```
...
cp "$old"/machine/profiles/local.toml ~/.hub-william/machine/profiles/
cp "$old"/machine/state/applied.json  ~/.hub-william/machine/state/
~/.hub-william/machine/install.sh init
```
...
files, and zsh symlinks into `machine/catalog/` can prove ownership even after
...
- **Skill**: write `catalog/skills/<name>/SKILL.md`, then `./install.sh skill add <name>`.
