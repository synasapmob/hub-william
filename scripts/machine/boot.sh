...
# Only machine/. Cone mode keeps every file at the repo root as well, which
# leaves ~/.hub-william looking like the JS project it is not part of.
# The installer and the documents it installs. They are separate trees on
# purpose — `scripts/` is the program, `contributors/` is what it publishes —
# so the checkout has to name both or an install arrives with no catalogue.
sparse() {
...
[ -d "$HOME_DIR/scripts/machine" ] || die "no scripts/machine in $HOME_DIR — wrong branch?"

INSTALL="$HOME_DIR/scripts/machine/install.sh"
