...
const CATALOG_ROOT = join(REPOSITORY_ROOT, "contributors");
const CATALOG_PREFIX = "contributors";
const OUT = "catalog";
 ...
 *   catalogue at once.
 * - `/catalog/collections/<owner>/<section>/<name>.zip` — one functional UI
 *   collection, even when its files live in several source folders.
...
export function catalogEndpoint(): Plugin {
  return {
    ...
    generateBundle() {
      ...
      const collections = new Map<string, CatalogFile[]>();

      for (const file of files) {
        const id = collectionIdForPath(file.path);
        ...
        if (!id || !owner || !section) continue;

        const key = `${owner}/${section}/${id}`;
      ...
      const collectionArchives = [...collections.entries()]
        ...
        .map(([key, members]) => {
          ...
          const zip = zipSync(entries, { level: 9, mtime: ARCHIVE_MTIME });
          const path = `collections/${key}.zip`;
        ...
        source: `${JSON.stringify(
          {
            ...
            archives,
            collections: collectionArchives,
          },
          null,
          2,
        )}\n`,
