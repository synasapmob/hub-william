...
[`apps/frontend/docs/architecture.md`](../../docs/architecture.md).
...
On a machine that has none of this yet:
...
```sh
~/.hub-william/apps/frontend/scripts/machine/install.sh init       # browse the catalog and pick
~/.hub-william/apps/frontend/scripts/machine/install.sh update     # pull, then sync
~/.hub-william/apps/frontend/scripts/machine/install.sh mcp auth   # log in to the OAuth servers
~/.hub-william/apps/frontend/scripts/machine/install.sh status     # catalog, CLIs, profile, pending
```

There is deliberately no `hub-william` package to install first. The public
Python script is a dependency-free bootstrap into the real installer.

Nothing here installs an agent CLI or opens a desktop GUI.

## Where it lives

`apps/frontend/public/install.py` clones into **`~/.hub-william`**, sparse and blobless, so
only `apps/frontend/scripts/` and `contributors/` land in the working tree —
the browser application is not part of a machine install.

That one line is both install and update: it clones when there is nothing there
and pulls when there is. It runs `init` by default. The same bootstrap can
install project-local Documents or selected MCP products:
...
cp "$old"/apps/frontend/scripts/machine/profiles/local.toml ~/.hub-william/apps/frontend/scripts/machine/profiles/
cp "$old"/apps/frontend/scripts/machine/state/applied.json  ~/.hub-william/apps/frontend/scripts/machine/state/
~/.hub-william/apps/frontend/scripts/machine/install.sh init
```
