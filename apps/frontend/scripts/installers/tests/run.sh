...
#
# Keep the standalone installers compatible with each available Python version.

set -uo pipefail

TESTS_DIR="$(cd -P "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
INSTALLERS_DIR="$(dirname "$TESTS_DIR")"
...
while [ $index -lt ${#pythons[@]} ]; do
  ...
  if ! (cd "$INSTALLERS_DIR" && "$binary" -m unittest discover -s tests -p 'test_*.py' -v "$@"); then
    status=1
  fi
done
